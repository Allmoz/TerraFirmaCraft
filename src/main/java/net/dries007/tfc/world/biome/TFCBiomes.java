...
public final class TFCBiomes
{
    ...
    // Aquatic biomes
    public static final BiomeExtension OCEAN = register("ocean", builder().heightmap(seed -> BiomeNoise.ocean(seed, -26, -12)).surface(ShoreAndOceanSurfaceBuilder.OCEAN).aquiferHeightOffset(-24).salty().type(BiomeBlendType.OCEAN).stratovolcanoes(1, -30, 180).noRivers()); // Ocean biome found near continents.
    public static final BiomeExtension OCEAN_REEF = register("ocean_reef", builder().heightmap(seed -> BiomeNoise.ocean(seed, -16, -8)).surface(ShoreAndOceanSurfaceBuilder.OCEAN).aquiferHeightOffset(-24).salty().type(BiomeBlendType.OCEAN).stratovolcanoes(1, -30, 180).noRivers()); // Ocean biome with reefs depending on climate. Could be interpreted as either barrier, fringe, or platform reefs.
    public static final BiomeExtension DEEP_OCEAN = register("deep_ocean", builder().heightmap(seed -> BiomeNoise.ocean(seed, -30, -16)).surface(ShoreAndOceanSurfaceBuilder.OCEAN).aquiferHeightOffset(-24).type(BiomeBlendType.OCEAN).salty().stratovolcanoes(1, -30, 180).noRivers()); // Deep ocean biome covering most all oceans.
    public static final BiomeExtension DEEP_OCEAN_TRENCH = register("deep_ocean_trench", builder().heightmap(seed -> BiomeNoise.oceanRidge(seed, -30, -16)).surface(ShoreAndOceanSurfaceBuilder.OCEAN).aquiferHeightOffset(-24).type(BiomeBlendType.OCEAN).salty().stratovolcanoes(1, -30, 180).noRivers()); // Deeper ocean with sharp relief carving to create very deep trenches
    ...
    public static final BiomeExtension PLATEAU = register("plateau", builder().heightmap(seed -> BiomeNoise.hills(seed, 20, 30)).surface(NormalSurfaceBuilder.INSTANCE).spawnable().type(RiverBlendType.TALL_CANYON).stratovolcanoes(1, 22, 180).noSandyRiverShores()); // Very high area, very flat top.
    public static final BiomeExtension PLATEAU_WIDE = register("plateau_wide", builder().heightmap(seed -> BiomeNoise.hills(seed, 20, 30)).surface(NormalSurfaceBuilder.INSTANCE).spawnable().type(RiverBlendType.TALUS).noSandyRiverShores().stratovolcanoes(1, 22, 180)); // Very high area, very flat top.
    ...
    public static final BiomeExtension VOLCANIC_OCEANIC_MOUNTAIN_LAKE = register("volcanic_oceanic_mountain_lake", builder().heightmap(seed -> BiomeNoise.mountains(seed, -24, 50)).surface(ShoreAndOceanSurfaceBuilder.VOLCANIC_MOUNTAINS).stratovolcanoes(1, 22, 180).carving(BiomeNoise::undergroundLakes).salty().type(BiomeBlendType.LAKE).noRivers());

    public static final BiomeExtension PLATEAU_LAKE = register("plateau_lake", builder().heightmap(seed -> BiomeNoise.hills(seed, 20, 30)).surface(NormalSurfaceBuilder.INSTANCE).carving(BiomeNoise::undergroundLakes).stratovolcanoes(1, 22, 180).type(BiomeBlendType.LAKE).noRivers());
    ...
    public static final BiomeExtension BURREN_PLATEAU = register("burren_plateau", builder().heightmap(seed -> BiomeNoise.burren(seed, BiomeNoise.hills(seed, 22, 32), 1.4)).surface(BurrenSurfaceBuilder.INSTANCE).spawnable().type(RiverBlendType.TALL_CANYON).stratovolcanoes(1, 22, 180).noSandyRiverShores()); // Plateau
